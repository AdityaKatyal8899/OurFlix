...
async function uploadPeepsFolder() {
  const folderPath = path.resolve('public/Peeps')
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (stat.isFile()) {
        ...
        const s3Key = `Peeps/${file}` // Keep folder prefix
    ...
    console.log('\nAll files from Peeps directory uploaded successfully!')
  } catch (error) {
    console.error('Peeps folder upload failed:', error)
  }
}
