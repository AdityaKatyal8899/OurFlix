...
async function uploadHerFolder() {
  const folderPath = path.resolve('public/Her')
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (stat.isFile()) {
        ...
        const s3Key = `Her/${file}` // Keep folder prefix
    ...
    console.log('\nAll files from Her directory uploaded successfully!')
  } catch (error) {
    console.error('Her folder upload failed:', error)
  }
}
