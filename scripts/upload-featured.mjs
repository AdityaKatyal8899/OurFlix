...
async function uploadFeaturedFolder() {
  const folderPath = path.resolve('public/Featured')
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (stat.isFile()) {
        ...
        const s3Key = `Featured/${file}` // Keep folder prefix
    ...
    console.log('\nAll files from Featured directory uploaded successfully!')
  } catch (error) {
    console.error('Featured folder upload failed:', error)
  }
}

uploadFeaturedFolder()
